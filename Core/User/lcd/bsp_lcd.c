...
static uint8_t EnableLayers = 0; //使能的层数
static uint32_t ActiveLayer = 0;
static LCD_DrawPropTypeDef DrawProp[MAX_LAYER_NUMBER];
static LCD_PreloadTypeDef PreloadTmp = {0, 0, 0, PRELOAD_DFT};
static uint8_t FormatSize[8] = {4, 3, 2, 2, 2, 2, 2, 2};
/**
...
#define HSW 1 //HSYNC宽度,最小显示单位
#define VSW 1 //VSYNC宽度,最小显示单位
...
static void LCD_GPIO_Config(void)
{
  ...
  LTDC_R3_GPIO_CLK_ENABLE();
  LTDC_R4_GPIO_CLK_ENABLE();
  LTDC_R5_GPIO_CLK_ENABLE();
  LTDC_R6_GPIO_CLK_ENABLE();
  LTDC_R7_GPIO_CLK_ENABLE();
  LTDC_G0_GPIO_CLK_ENABLE();
  LTDC_G1_GPIO_CLK_ENABLE();
  LTDC_G2_GPIO_CLK_ENABLE();
  LTDC_G3_GPIO_CLK_ENABLE();
  LTDC_G3_GPIO_CLK_ENABLE();
  LTDC_G5_GPIO_CLK_ENABLE();
  LTDC_G6_GPIO_CLK_ENABLE();
  LTDC_G7_GPIO_CLK_ENABLE();
  LTDC_B0_GPIO_CLK_ENABLE();
  LTDC_B1_GPIO_CLK_ENABLE();
  LTDC_B2_GPIO_CLK_ENABLE();
  LTDC_B3_GPIO_CLK_ENABLE();
  LTDC_B4_GPIO_CLK_ENABLE();
  LTDC_B5_GPIO_CLK_ENABLE();
  LTDC_B6_GPIO_CLK_ENABLE();
  LTDC_B7_GPIO_CLK_ENABLE();
  ...
  if (1)
  {
    ...
    BL_PWM_Handler.Instance = TIM14;
    BL_PWM_Handler.Init.Prescaler = 84 - 1;
    BL_PWM_Handler.Init.CounterMode = TIM_COUNTERMODE_UP;
    BL_PWM_Handler.Init.Period = 1000;
    BL_PWM_Handler.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    BL_PWM_Handler.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    if (HAL_TIM_Base_Init(&BL_PWM_Handler) != HAL_OK)
    {
      Error_Handler();
    }
    if (HAL_TIM_PWM_Init(&BL_PWM_Handler) != HAL_OK)
    {
      Error_Handler();
    }
    ...
    if (HAL_TIM_PWM_ConfigChannel(&BL_PWM_Handler, &sConfigOC, TIM_CHANNEL_1) != HAL_OK)
    {
      Error_Handler();
    }
    ...
    GPIO_InitStruct.Pull = GPIO_PULLUP;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.Alternate = LTDC_BL_AF;
    HAL_GPIO_Init(LTDC_BL_GPIO_PORT, &GPIO_InitStruct);

    LTDC_BL_TIM_CLK_ENABLE();
  }
#endif
}

/**
 * @description: 初始化LTDC外设及其刷新速度、中断
 * @param {*}
 * @return {*}
 */
void LCD_Init(void)
{
  ...
  periph_clk_init_struct.PeriphClockSelection = RCC_PERIPHCLK_LTDC;
  periph_clk_init_struct.PLLSAI.PLLSAIN = 50;
...
/**
 * @description: LTDC行刷新中断回调函数
 * @param {LTDC_HandleTypeDef} *hltdc
 * @return {*}
 */
void HAL_LTDC_LineEventCallback(LTDC_HandleTypeDef *hltdc)
{
  if (hltdc == (Ltdc_Handler))
  {
#if DMA2D_SWITCH
    if (HAL_DMA2D_GetState(&Dma2d_Handler) == HAL_DMA2D_STATE_BUSY || HAL_DMA2D_GetState(&Dma2d_Handler) == HAL_DMA2D_STATE_TIMEOUT)
    {
...
{
#if DMA2D_SWITCH
  HAL_DMA2D_MspInit(&Dma2d_Handler);
  ...
  HAL_DMA2D_MspDeInit(&Dma2d_Handler);
#else
  uint16_t Xpos = ((uint32_t)pDst - (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress)) / FormatSize[LCD_GetPixelFormat()] % LCD_GetXSize();
  uint16_t Ypos = ((uint32_t)pDst - (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress)) / FormatSize[LCD_GetPixelFormat()] / LCD_GetXSize();

  for (uint32_t cntY = Ypos; cntY < (Ypos + ySize); cntY++)
  {
    for (uint32_t cntX = Xpos; cntX < (Xpos + xSize); cntX++)
    {
      LCD_DrawPixel(cntX, cntY, ColorIndex);
    }
  }
#endif
}
...
static void LL_ConvertLineToARGB8888(uint32_t LayerIndex, void *pSrc, void *pDst, uint32_t xSize, uint32_t ColorMode)
{
  ...
  Dma2d_Handler.LayerCfg[LayerIndex].AlphaMode = DMA2D_NO_MODIF_ALPHA;
  Dma2d_Handler.LayerCfg[LayerIndex].InputAlpha = 0xFF;
  Dma2d_Handler.LayerCfg[LayerIndex].InputColorMode = ColorMode;
  Dma2d_Handler.LayerCfg[LayerIndex].InputOffset = 0;
...
/**
 * @description: 获取指定页的格式
 * @param {uint8_t} LayerIndex 指定层 0：BG 1：FG
 * @return {*}
 */
uint32_t LCD_GetPixelFormat(void)
{
  return Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat;
}

/**
 * @description: 获取指定页的页长度限制
 * @param {uint8_t} LayerIndex 指定层 0：BG 1：FG
 * @return {*}
 */
uint32_t LCD_GetPageSize(void)
{
  return LCD_GetXSize() * LCD_GetYSize() * FormatSize[Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat];
}
 ...
 * @return {*}
 */
uint32_t LCD_GetSceneAddress(uint8_t SceneIndex)
{
  return LCD_FB_START_ADDRESS + SceneIndex * LCD_GetPageSize();
}
...
#if !(BGLIGHT_SWITCH)
  HAL_GPIO_WritePin(LTDC_BL_GPIO_PORT, LTDC_BL_GPIO_PIN, GPIO_PIN_RESET); /* 开背光*/
#else
  HAL_TIM_Base_Start(&BL_PWM_Handler);
  HAL_TIM_PWM_Start(&BL_PWM_Handler, LTDC_BL_PWM_CHANNEL);
#endif
}
...
#if !(BGLIGHT_SWITCH)
  HAL_GPIO_WritePin(LTDC_BL_GPIO_PORT, LTDC_BL_GPIO_PIN, GPIO_PIN_SET); /*关背光*/
#else
  HAL_TIM_Base_Stop(&BL_PWM_Handler);
  HAL_TIM_PWM_Stop(&BL_PWM_Handler, LTDC_BL_PWM_CHANNEL);
#endif
}

/**
 * @description: 设置背光亮度
 * @param {uint16_t} lightLevel 亮度等级 20-100
 * @return {*}
 */
void LCD_SetBGLight(uint16_t lightLevel)
{
  if (lightLevel > LTDC_BL_PWM_MAX)
  {
    lightLevel = LTDC_BL_PWM_MAX;
  }
  else if (lightLevel < LTDC_BL_PWM_MIN)
  {
    lightLevel = LTDC_BL_PWM_MIN;
  }
  lightLevel *= 10;
  __HAL_TIM_SET_COMPARE(&BL_PWM_Handler, TIM_CHANNEL_1, lightLevel);
}
...
/**
  * @brief  设置LCD缓冲帧的首地址，并开始缓冲
  * @param  LayerIndex: 前景层(层1)或者背景层(层0)
  * @param  Address: LCD缓冲帧的首地址     
  * @retval 无
  */
void LCD_SetLayerAddress_Reload(uint32_t LayerIndex, uint32_t Address)
{
...
/**
 * @description: 设置LCD预加载场景区
 * @param {uint32_t} LayerIndex (0:BG,1:FG)
 * @param {uint32_t} preLoadScene (场景序号)
 * @return {*}
 */
void LCD_SetPreLoadScene(uint32_t LayerIndex, uint32_t preLoadScene)
{
  uint32_t pageSize;
  LayerIndex = (LayerIndex > LTDC_LAYER_FG) ? LTDC_LAYER_FG : LayerIndex;
  if ((PreloadTmp.State & PRELOAD_RDY) && PreloadTmp.SelLayer != LayerIndex)
  {
    PreloadTmp.State |= PRELOAD_ERR;
    return;
  }
  LCD_SelectLayer(LayerIndex);
  pageSize = LCD_GetXSize() * LCD_GetYSize() * FormatSize[Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat];
  if (((preLoadScene + 1) * pageSize) >= LCD_FB_FIFO_SIZE)
  {
    PreloadTmp.State |= PRELOAD_OVER;
    return;
  }

  if (PreloadTmp.State & PRELOAD_RDY)
  {
    PreloadTmp.SelLayer = LayerIndex;
    PreloadTmp.AimSence = preLoadScene;
    Ltdc_Handler.LayerCfg[LayerIndex].FBStartAdress = LCD_FB_START_ADDRESS + pageSize * preLoadScene;
    PreloadTmp.State = PRELOAD_RDY;
    return;
  }
  else
  {
    PreloadTmp.SelLayer = LayerIndex;
    PreloadTmp.OriSence = ((Ltdc_Handler.LayerCfg[LayerIndex].FBStartAdress - LCD_FB_START_ADDRESS) / pageSize);
    PreloadTmp.AimSence = preLoadScene;
    Ltdc_Handler.LayerCfg[LayerIndex].FBStartAdress = LCD_FB_START_ADDRESS + pageSize * preLoadScene;
    PreloadTmp.State = PRELOAD_RDY;
    return;
 ...
 * @return {*}
 */
void LCD_ResetPreLoadScene(void)
{
  uint32_t pageSize;
  pageSize = LCD_GetXSize() * LCD_GetYSize() * FormatSize[Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat];
  if ((PreloadTmp.State & PRELOAD_RDY) || (PreloadTmp.State & PRELOAD_ERR))
  {
    Ltdc_Handler.LayerCfg[PreloadTmp.SelLayer].FBStartAdress = LCD_FB_START_ADDRESS + pageSize * PreloadTmp.OriSence;
    LCD_SetLayerAddress_Reload(PreloadTmp.SelLayer, Ltdc_Handler.LayerCfg[PreloadTmp.SelLayer].FBStartAdress);
    PreloadTmp.OriSence = 0;
    PreloadTmp.AimSence = 0;
    PreloadTmp.SelLayer = 0;
    PreloadTmp.State = PRELOAD_DFT;
  }
}
...
/**
  ...
  * @retval 无
  */
void LCD_CopyScene(uint8_t srcScene, uint8_t dirScene)
{
  volatile uint32_t *pSdramAddressDir = (uint32_t *)LCD_GetSceneAddress(dirScene);
  volatile uint32_t *pSdramAddressSrc = (uint32_t *)LCD_GetSceneAddress(srcScene);
  memcpy((uint32_t *)pSdramAddressDir, (const uint32_t *)pSdramAddressSrc, LCD_GetPageSize());
}
...
/**
 * @description: 开关指定层
 * @param {uint8_t} LayerIndex 层号;0,第一层;1,第二层;
 * @param {uint8_t} sw 1,打开;0,关闭
 * @return {*}
 */
void LCD_Layer_Switch(uint8_t LayerIndex, uint8_t sw)
{
  if (LayerIndex > 1)
    return;
  if (sw == 1)
  ...
  HAL_LTDC_Reload(&Ltdc_Handler, 1); //重新加载配置
}
...
uint32_t LCD_ReadPixel(uint16_t Xpos, uint16_t Ypos)
{
  ...
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(volatile uint32_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2 * (Ypos * LCD_GetXSize() + Xpos)));
  }
  else if (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = (*(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos)) + 2) & 0x00FFFFFF);
    ret |= (*(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos)) + 1) & 0x00FFFFFF);
    ret |= (*(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos))) & 0x00FFFFFF);
  }
  ...
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(volatile uint16_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2 * (Ypos * LCD_GetXSize() + Xpos)));
  }
  else
  {
    /* 从SDRAM显存中读取颜色数据 */
    ret = *(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2 * (Ypos * LCD_GetXSize() + Xpos)));
  }
...
void LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
  ...
  if (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    *(volatile uint32_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (4 * (Ypos * LCD_GetXSize() + Xpos))) = RGB_Code;
  }
  else if (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    *(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos)) + 2) = 0xFF & (RGB_Code >> 16);
    *(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos)) + 1) = 0xFF & (RGB_Code >> 8);
    *(volatile uint8_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (3 * (Ypos * LCD_GetXSize() + Xpos))) = 0xFF & RGB_Code;
  }
  else if ((Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB565) ||
           (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB4444) ||
           (Ltdc_Handler.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_AL88))
  {
    *(volatile uint16_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2 * (Ypos * LCD_GetXSize() + Xpos))) = (uint16_t)RGB_Code;
  }
  else
  {
    *(volatile uint16_t *)(Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress + (2 * (Ypos * LCD_GetXSize() + Xpos))) = (uint16_t)RGB_Code;
...
{
  LCD_DrawChar(Xpos, Ypos, &DrawProp[ActiveLayer].pFont->table[(Ascii - ' ') * DrawProp[ActiveLayer].pFont->Height * ((DrawProp[ActiveLayer].pFont->Width + 7) / 8)]);
}
...
{
  uint32_t Xaddress = 0;
  Xaddress = (Ltdc_Handler.LayerCfg[ActiveLayer].FBStartAdress) + FormatSize[LCD_GetPixelFormat()] * (LCD_GetXSize() * Ypos + Xpos);
  /* 填充数据 */
...
void LCD_FillRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  /* 填充矩形 */
  LL_FillBuffer(ActiveLayer, (uint32_t *)Xaddress, Width, Height, (LCD_GetXSize() - Width), DrawProp[ActiveLayer].TextColor);
}
 ...
 * @return {*}
 */
void LCD_TestLayerInit(void)
{

  LCD_DisplayOff();
  // LCD_LayerInit(LTDC_LAYER_BG, LCD_FB_START_ADDRESS, ARGB8888);
  // LCD_SelectLayer(LTDC_LAYER_BG);
  // LCD_Clear(LCD_COLOR_GREEN);
  // LCD_SetTransparency(LTDC_LAYER_BG, 255);
  LCD_LayerInit(LTDC_LAYER_FG, LCD_FB_START_ADDRESS + (LCD_GetXSize() * LCD_GetYSize() * FormatSize[ARGB8888]), ARGB8888);
  LCD_SelectLayer(LTDC_LAYER_FG);
  LCD_Clear(LCD_COLOR_BLACK);
  LCD_SetTransparency(LTDC_LAYER_FG, 255);
...
void LCD_Test2(void)
{
  ...
  LCD_SetPreLoadScene(LTDC_LAYER_FG, 2);
  LCD_Clear(LCD_COLOR_YELLOW);
  LCD_SetTextColor(LCD_COLOR_RED);
  LCD_FillRect(100, 100, 600, 280);
  LCD_SetTextColor(LCD_COLOR_BLUE);
  LCD_DrawRect(0, 0, 800, 480);
  LCD_DrawRect(1, 1, 798, 478);
  LCD_DrawRect(2, 2, 796, 476);
  LCD_LoadScene();
  LCD_SetPreLoadScene(LTDC_LAYER_FG, 1);

  LCD_CopyScene(2, 1);
  LCD_FillRect(200, 200, 200, 100);
  // LCD_Clear(LCD_COLOR_GREEN);
  // LCD_SetTextColor(LCD_COLOR_BLUE);
  // LCD_FillRect(100, 100, 600, 280);
  // LCD_SetTextColor(LCD_COLOR_RED);
  ...
  while (1)
  {
    testCNT = !testCNT;
    HAL_Delay(1000);
    LCD_SetPreLoadScene(LTDC_LAYER_FG, testCNT ? 1 : 2);
    LCD_LoadScene();
  }
}
...
void LCD_Test3(void)
{
  ...
  while (1)
  {
    ...
    LCD_FillRect(100, 100, 600, 280);
    LCD_SetTextColor(LCD_COLOR_RED);
    LCD_DrawRect(0, 0, 799, 479);
    LCD_DrawRect(1, 1, 797, 477);
    LCD_DrawRect(2, 2, 795, 475);
